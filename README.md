...
Методы класса:*
- setCatalog(value: string) - устанавливает каталог продуктов
- addToBasket() - добавляет продукт в корзину
- removeFromBasket(value: string) - удаляет продукт из корзины
- getTotalBasketPrice(value: number) - считает итоговую стоимость товаров в корзине
- getEmptyOrder(value: string) - устанавливает пустые поля в формах оформления заказа
- getCountProductInBasket(value: number) - считает количество товаров в корзине
- setOrderFields(value: number) - устанавливает значения в форму заказа
- validateOrder(value: boolean) - проверяет внесены ли данные в поля формы с оплатой
- validateContacts(value: boolean) - проверяет внесены ли данные в поля формы с контактами
...
### class Card
Класс отвечает за отрисовку карточки товара, наследуется от базового класса Component и расширяется интерфейсом IProduct. Данные класса служат для отображения карточки и информации в ней в галлерее товаров, просмотре карточки товара и корзине.
...
### class Basket
Класс наследуется от базового класса Component и расширяется интерфейсом IBasket. Отображает информация по товарам, показывает общую стоимость и дает возможность начать оформлять.
